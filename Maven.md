...
title: Maven
tags: [Maven]
index_img: /img/article/maven.png
date: 2022-10-08 11:03:00
---
...
![images](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/img006.ab4f2e31.png)
...
![images](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/img010.74e515e5.png)
...
![./images](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/img003.f9cc536c.png)
...
![image-20220903123601616](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20220903123601616.png)

## 2、解压Maven核心程序

核心程序压缩包：apache-maven-3.8.6-bin.zip，解压到**非中文、没有空格**的目录。例如：

![image-20220903131327866](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20220903131327866.png)
...
![image-20221006131652590](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006131652590.png)

本地仓库默认值：用户家目录/.m2/repository。由于本地仓库的默认位置是在用户的家目录下，而家目录往往是在 C 盘，也就是系统盘。将来 Maven 仓库中 jar 包越来越多，仓库体积越来越大，可能会拖慢 C 盘运行速度，影响系统性能。所以建议将 Maven 的本地仓库放在其他盘符下。配置方式如下：

仓库地址下的conf->setting.xml
...
<localRepository>D:\@localwarehouse\maven_repository</localRepository>
```

![image-20220903143401784](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20220903143401784.png)
...
![image-20220903144301119](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20220903144301119.png)
...
![image-20220903144523153](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20220903144523153.png)
...
![./images](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/img030.7f885903.png)

![./images](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/img032.9289c76e.png)
...
![images](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/img004.e823213d.png)
...
![./images](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/img033.39a65cee.png)
...
![image-20221006132327772](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006132327772.png)

查看 target 目录，class 文件已生成，编译完成。

![image-20221006132349285](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006132349285.png)
...
![image-20221006132605451](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006132605451.png)
...
![image-20221006192307534](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006192307534.png)
...
![image-20221006194504680](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006194504680.png)
...
​	servlet-api 、jsp-api ------- provided （编译、测试 有效)

运行时无效 防止和 tomcat 下 jar 冲突）
...
![image-20221006214835045](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006214835045.png)

会发现出现除了 spring-webmvc 以外的其他 jar。因为我们的项目依赖 spring-webmv.jar，而spring-webmv.jar 会依赖 spring-beans.jar 等等，所以 spring-beans.jar 这些 jar 包也出现在了我

们的 maven 工程中，这种现象我们称为依赖传递。从下图中可看到他们的关系：（请注意spring-beans 的版本）

![image-20221006214918099](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006214918099.png)

## 8.2依赖冲突的解决

接着添加一个依赖

![image-20221006215003614](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006215003614.png)

我们会发现这两个 jar 包同时都依赖了 spring-beans

![image-20221006215024013](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006215024013.png)

但是spring-webmvc 依赖 spirng-beans-4.2.4，spring-context 依赖 spring-beans-5.0.2，但是发现spirng-beans-4.2.4 加入到工程中

![image-20221006215055983](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006215055983.png)
...
![image-20221006215404927](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006215404927.png)
...
![image-20221006215618898](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006215618898.png)
...
![image-20221006215839099](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006215839099.png)

还可以把版本号提取出来，使用<properties>标签设置成变量。

![image-20221006220050555](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006220050555.png)

注意：在工程中锁定依赖的版本并不代表在工程中添加了依赖，如果工程需要添加锁定版本的依赖则需要单独添加<dependencies></dependencies>标签，如下：

![image-20221006220314313](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221006220314313.png)
...
创建mapper类对应的xml文件时，包名要一样。resources下的com.ep.mapper。不能用com.ep.mapper来创建文件夹，应该使用com/ep/mapper

![image-20221007124112946](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007124112946.png)

![image-20221007124421144](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007124421144.png)
...
![image-20221007194247360](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007194247360.png)

## 创建父工程

![image-20221007192745567](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007192745567.png)

## 创建子工程

![image-20221007193655130](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007193655130.png)

![image-20221007193731793](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007193731793.png)

![image-20221007194005953](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007194005953.png)
...
web中引入service.service中引入mapper

![image-20221007202432988](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007202432988.png)

web中导入mapper和service的配置文件

![image-20221007210749746](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007210749746.png)

## 三种启动方式

1.父工程 

![image-20221007213004185](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007213004185.png)

2. maven_parent 中安装

![](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007213422629.png)

在maven_web 中运行

![image-20221007213558496](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007213558496.png)

3. 配置外部的tomcat

![image-20221007214028778](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007214028778.png)

## 依赖范围对传递依赖

是因为依赖会有依赖范围，依赖范围对传递依赖也有影响，例如有 A、B、C，A 依赖 B、B依赖 C，C 可能是 A 的传递依赖，如下图：

![image-20221007214627071](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007214627071.png)
...
![image-20221007214849760](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007214849760.png)
...
![image-20221007215155561](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007215155561.png)
...
![image-20221007220105624](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007220105624.png)
...
![image-20221007220308799](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007220308799.png)
...
![image-20221007220938045](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007220938045.png)

安装成功之后，可以进入服务里查看

![image-20221007221018358](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007221018358.png)
...
![image-20221007221754320](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007221754320.png)

方法 2：

直接启动 nexus 服务

![image-20221007221357564](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007221357564.png)

查看 nexus 的配置文件 conf/nexus.properties

![image-20221007221512757](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007221512757.png)
...
![image-20221007221822232](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007221822232.png)
...
![image-20221007222039520](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007222039520.png)
...
![image-20221007222321084](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007222321084.png)

**central**：代理仓库，代理中央仓库

![image-20221007223001918](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007223001918.png)
...
需要在客户端即部署 ssm_dao 工程的电脑上配置 maven环境，并**修改** **settings.xml**
...
![image-20221007223546667](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007223546667.png)
...
maven_mapper下的pom.xml（因为要上传maven_mapper到私服）
...
![image-20221007223954336](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007223954336.png)

注意：pom.xml 这里<id> 和 settings.xml 配置 <id> 对应！

执行deploy命令上传到私服

![image-20221007224034573](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007224034573.png)

也可以点击这里查看

![image-20221007224355552](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007224355552.png)



![image-20221007224341422](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221007224341422.png)
...
![image-20221008094414705](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221008094414705.png)
...
![image-20221008095014492](https://trpora-1300527744.cos.ap-chongqing.myqcloud.com/img/image-20221008095014492.png)
...
----打开cmd直接运行
mvn install:install-file -DgroupId=com.alibaba -DartifactId=fastjson -Dversion=1.1.37 -Dpackaging=jar -Dfile=D:\@黑马java\黑马JavaEE 57期\00 讲义+笔记+资料\主流框架\35.会员版(2.0)-就业课(2.0)-Maven高级\maven高级\资料\安装第三方jar包\fastjson-1.1.37.jar
```
...
----打开cmd直接运行
mvn deploy:deploy-file -DgroupId=com.alibaba -DartifactId=fastjson -Dversion=1.1.37 -Dpackaging=jar -Dfile=D:\@黑马java\黑马JavaEE 57期\00 讲义+笔记+资料\主流框架\35.会员版(2.0)-就业课(2.0)-Maven高级\maven高级\资料\安装第三方jar包\fastjson-1.1.37.jar -Durl=http://localhost:8081/nexus/content/repositories/thirdparty/ -DrepositoryId=thirdparty
```
